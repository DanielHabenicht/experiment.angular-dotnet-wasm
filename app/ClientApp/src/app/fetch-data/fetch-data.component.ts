...
import { HelloWorld } from 'src/assets/lib-wasm/dotnet';
...
export class FetchDataComponent {
  public forecasts: HelloWorld.WeatherForecast[] = [];
  public model: any = {
  ...
  constructor(private http: HttpClient, @Inject('BASE_URL') private baseUrl: string, private wasmService: LibWasmService) {
    http.get<HelloWorld.WeatherForecast[]>(baseUrl + 'weatherforecast').subscribe(result => {
  ...
  public validate(){
    ...
    this.http.post<HelloWorld.WeatherForecast[]>(this.baseUrl + 'weatherforecast', this.form.getRawValue()).subscribe(result => {
    ...
    for(let result of validationResult){
        if(result.memberNames.length > 0){
            this.form.controls[result.memberNames[0]].setErrors({'error': result.errorMessage})
        }else {
